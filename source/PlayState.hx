...
class PlayState extends MusicBeatState
{
	...
	var phillyGlowFunction:Void->Void = null;
	...
	public var customPhillyInsert:Int = 0;
	private var cityTunnel:Bool = false;

	var upperBlackBar:FlxSpriteExtra;
	var bottomBlackBar:FlxSpriteExtra;
	...
	override public function create()
	{
		...
		var file:String = Paths.jsonSong(songName + '/dialogue'); //Checks for json/Psych Engine dialogue
		...
		var heightSub = 18;
		var widthSub = 90;
		healthBar = new FlxBar(0, healthBarBG.y + heightSub/2, RIGHT_TO_LEFT, Std.int(healthBarBG.width - widthSub), Std.int(healthBarBG.height - heightSub), this,
			'health', 0, 2);
		...
		healthBarBG.xAdd = (healthBar.width - healthBarBG.width) / 2 + 4;
		healthBarBG.yAdd = (healthBar.height - healthBarBG.height) / 2;
		...
		var foldersToCheck:Array<String> = [
			SUtil.getPath() + Paths.getPreloadPath('songs/' + formattedSong + '/'),
			SUtil.getPath() + Paths.getPreloadPath('data/' + formattedSong + '/')
		];

		#if MODS_ALLOWED
		foldersToCheck.insert(0, Paths.mods('data/' + formattedSong + '/'));
		foldersToCheck.insert(0, Paths.mods('songs/' + formattedSong + '/'));
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0) {
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/data/' + formattedSong + '/'));
			foldersToCheck.insert(0, Paths.mods(Paths.currentModDirectory + '/songs/' + formattedSong + '/'));
		}
		#end
		...
		if(formattedSong == 'streetstyle')
			midSongVideo('CUTSCENE_STREETSTYLE', 21.6);
		else
			startCountdown();
		RecalculateRating();

		//PRECACHING MISS SOUNDS BECAUSE I THINK THEY CAN LAG PEOPLE AND FUCK THEM UP IDK HOW HAXE WORKS
		if(ClientPrefs.hitsoundVolume > 0) Paths.cacheSound('hitsound', 'sound');
		Paths.cacheSound('missnote1', 'sound');
		Paths.cacheSound('missnote2', 'sound');
		Paths.cacheSound('missnote3', 'sound');

		if (PauseSubState.songName != null) {
			Paths.cacheSound(PauseSubState.songName, 'music');
		} else if(ClientPrefs.pauseMusic != 'None') {
			Paths.cacheSound(Paths.formatToSongPath(ClientPrefs.pauseMusic), 'music');
		}
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			inCutscene = true;
			Paths.cacheSound('dialogue', 'sound');
			Paths.cacheSound('dialogueClose', 'sound');
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			countdownSpr.frames = Paths.getSparrowAtlas('countdown');
			countdownSpr.antialiasing = ClientPrefs.globalAntialiasing;
			countdownSpr.scrollFactor.set();
			add(countdownSpr);
			countdownSpr.animation.finishCallback = function(name:String)
			{
				if (name == 'go')
				{
					countdownSpr.kill();
			...
			var anims:Array<String> = ['go', 'one', 'two', 'three'];
			for (anim in anims)
			{
				countdownSpr.animation.addByPrefix(anim, anim, 24, false);
			}
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					case 0:
						...
						countdownSpr.animation.play('three', true);
						countdownSpr.alpha = 1;
					case 1:
						FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), 0.6);
						countdownSpr.animation.play('two', true);
					case 2:
						FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), 0.6);
						countdownSpr.animation.play('one', true);
					case 3:
						FlxG.sound.play(Paths.sound('introGo' + introSoundsSuffix), 0.6);
						countdownSpr.animation.play('go', true);
				}
	...
	private function generateSong():Void
	{
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		var file:String = Paths.jsonSong(songName + '/events');
		#if sys
		if (FileSystem.exists(Paths.modsSongJson(songName + '/events')) || FileSystem.exists(file))
		#else
		if (OpenFlAssets.exists(file))
		#end
		{
		...
		switch(event.event) {
			...
			case 'Philly Glow':
				if (phillyGlowGradient == null) {
					phillyGlowGradient = new PhillyGlow.PhillyGlowGradient(-400, 350); //This shit was refusing to properly load FlxGradient so fuck it
					//phillyGlowGradient.visible = false;
					insert(members.indexOf(gfGroup) + customPhillyInsert, phillyGlowGradient);
					...
					if(!ClientPrefs.lowQuality)
					{
						phillyGlowFunction = function() {
							var particlesNum:Int = FlxG.random.int(8, 12);
							var width:Float = (3000 / particlesNum);
							var color:FlxColor = FlxColor.PINK;
							for (j in 0...3)
							{
			...
			case 'Subway Train':
				...
				if (trainGroup == null) {
					...
					insert(members.indexOf(gfGroup)-1+customPhillyInsert, trainGroup);

					var train:FlxSprite = new FlxSprite().loadGraphic(Paths.image("cutscene/train", "neo"));
				...
				if (doorExists < 1) {
					...
					switch (val1) {
						case 1:
							...
							door.frames = Paths.getSparrowAtlas("cutscene/doors1", "neo");
							door.animation.addByPrefix('open', 'DOORS OPENING 1', 24, false);
							door.animation.addByPrefix('close', 'DOORS CLOSING 1', 24, false);
							door.animation.addByIndices('idle', 'DOORS OPENING 1', [0], "", 24, false);
						case 2:
							door.setPosition(2486, 89);
							door.frames = Paths.getSparrowAtlas("cutscene/doors2", "neo");
							door.animation.addByPrefix('open', 'DOORS OPENING 2', 24, false);
							door.animation.addByPrefix('close', 'DOORS CLOSING 2', 24, false);
							door.animation.addByIndices('idle', 'DOORS OPENING 2', [0], "", 24, false);
						case 3:
							door.setPosition(2640, 158);
							door.frames = Paths.getSparrowAtlas("cutscene/doors3", "neo");
							door.animation.addByPrefix('open', 'DOORS OPENING 7', 24, false);
							door.animation.addByPrefix('close', 'DOORS CLOSING 7', 24, false);
							door.animation.addByIndices('idle', 'DOORS OPENING 7', [0], "", 24, false);
						case 4:
							door.setPosition(2485, 91);
							door.frames = Paths.getSparrowAtlas("cutscene/doors4", "neo");
							door.animation.addByPrefix('open', 'DOORS OPENING 6', 24, false);
							door.animation.addByPrefix('close', 'DOORS CLOSING  6', 24, false);
							door.animation.addByIndices('idle', 'DOORS OPENING 6', [0], "", 24, false);
					}
			...
			case 'Streetstyle Neon':
				addCharacterToList('bf-graffiti', 0);
				addCharacterToList('skarlet-graffiti', 1);
				addCharacterToList('skarlet-neonsolo', 1);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			var time:Float = 2000;//shit be werid on 4:3
		...
		if (generatedMusic)
		{
			...
			if(startedCountdown)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if(cpuControlled && daNote.mustPress) {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Subway Train':
				...
				// i couldve used a map for this i dont care its 9 days till the deadline
				// i just gotta make this code work - Grizzle
				var doors:FlxSprite = null;
				for (spr in doorArray) {
					if (spr.ID == val1) doors = spr;
					spr.alpha = 0;
				}
						...
						var timer1:FlxTimer = new FlxTimer().start(0.75, function(_) {
							...
							doors.animation.finishCallback = function(name:String) {
								if (name == "open") {
									...
									modchartTimers.set("trainCloseCooldown", timer2);
								} else if (name == "close") {
									var timer3:FlxTimer = new FlxTimer().start(0.75, function(_) {
										...
										modchartTweens.set("trainBye", tween2);
									});
									modchartTimers.set("trainByeCooldown", timer3);
								}
							}
						});
						modchartTimers.set("trainOpenCooldown", timer1);
					}
				});
				modchartTweens.set("trainArrive", tween1);
	...
	public function fussEnd()
	{
		endingSong = true;
		startVideo('AFTER_FUSS_CUTSCENE');
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if (SONG.notes[Math.floor(curBeat / 4)] != null)
			{
				if (SONG.notes[Math.floor(curBeat / 4)].altAnim && !SONG.notes[Math.floor(curBeat / 4)].gfSection) {
					altAnim = '-alt';
				}
			}
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					case 'Hurt Note': //Hurt note
						if(!note.noMissAnimation && boyfriend.animation.getByName('hurt') != null) {
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if(note != null) {
			...
			var hsv = ClientPrefs.arrowHSV[data % 4];
			hue = hsv[0] / 360;
			sat = hsv[1] / 100;
			brt = hsv[2] / 100;
		}
	...
	override function stepHit()
	{
		...
		if (canResync && (Math.abs(FlxG.sound.music.time - (Conductor.songPosition - Conductor.offset)) > 20
			|| (!vocalsFinished && SONG.needsVoices && Math.abs(vocals2.time && vocals1.time - (Conductor.songPosition - Conductor.offset)) > 20)))
		{
			resyncVocals();
		}
	...
	override function beatHit()
	{
		...
		if (generatedMusic)
		{
			...
			if (phillyGlowFunction != null) phillyGlowFunction();
		}
